...
enum _Panel { MAIN, SETTINGS, GUIDE, LOG }
...
func _unhandled_input(event: InputEvent) -> void:
	...
	# Menu is open: ESC backs out of sub-panels or closes from main.
...
func _build_settings_panel(vbox: VBoxContainer) -> void:
	...
	# Volume sliders (placeholder — visible/draggable; audio bus wiring deferred)
	_master_slider = _add_volume_row(vbox, "Master Volume",
		SettingsStore.master_volume, _on_master_changed)
	_music_slider  = _add_volume_row(vbox, "Music Volume",
		SettingsStore.music_volume,  _on_music_changed)
	_sfx_slider    = _add_volume_row(vbox, "SFX Volume",
...
func _rebuild_log_list() -> void:
	...
	# RogueFinder is the player template, not a collectable creature — always exclude.
	var all_ids: Array[String] = GameState.encountered_archetypes
	var display_ids: Array[String] = all_ids.filter(
		func(id: String) -> bool: return id != "RogueFinder"
	)

	if display_ids.is_empty():
		var empty := Label.new()
		empty.text = "No archetypes encountered yet."
		empty.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
		empty.add_theme_font_size_override("font_size", 14)
		_log_list.add_child(empty)
		return

	for id: String in display_ids:
		_log_list.add_child(_build_archetype_card(id))


func _build_archetype_card(id: String) -> Control:
	var arch: ArchetypeData = ArchetypeLibrary.get_archetype(id)
	...
	# "archer_bandit" → "Archer Bandit"
	name_lbl.text = id.replace("_", " ").capitalize()
