...
func test_round_trip_identity() -> void:
	...
	GameState.load_save()
	assert(GameState.party.size() == 1,
		"party should have 1 member after load, got %d" % GameState.party.size())
...
func test_dead_flag_persistence() -> void:
	_clean()
	GameState.init_party()
	GameState.party[0].is_dead = true
	GameState.save()
	GameState.reset()
	GameState.load_save()
	assert(GameState.party[0].is_dead == true,
		"is_dead on party[0] should survive round-trip")
...
func test_fresh_run_guard() -> void:
	...
	GameState.init_party()
	assert(GameState.party.size() == 1,
		"party should have 1 after first init_party(), got %d" % GameState.party.size())
	GameState.init_party()
	assert(GameState.party.size() == 1,
