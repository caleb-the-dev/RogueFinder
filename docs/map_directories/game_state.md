...
| `test_room_mode` | `bool` | `false` | Transient flag — set by MapManager dev panel "⚔ Test Room" button before transitioning to CombatScene3D. `CombatManager3D._setup_units()` reads it to spawn hardcoded test combatants instead of `GameState.party`. Cleared by `_end_combat()` on combat exit. **NOT saved to disk — never serialized.** |
...
| `party` | `GameState.party` | JSON Array of dicts — each dict holds all scalar fields + `abilities`/`ability_pool` as string arrays + `feat_ids` as string array + `level`/`xp`/`pending_level_ups` as ints + `physical_armor`/`magic_armor` as ints + `weapon_id`/`armor_id`/`accessory_id` as strings. Deserialized back to `Array[CombatantData]` by `_deserialize_combatant()`. **Migration:** old saves with `armor_defense` key (no `physical_armor`) set both `physical_armor = armor_defense` and `magic_armor = armor_defense`. Missing `level`/`xp`/`pending_level_ups` default to `1`/`0`/`0`. |
