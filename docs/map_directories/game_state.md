...
| `inventory` | `Array` | `[]` | Shared party bag. Holds raw reward dicts `{id, name, description, item_type, seen, rarity}` for both equipment and consumables. `item_type` filters into tabs (Stage 2). `seen: bool` drives the new-item glow in PartySheet — `add_to_inventory()` stamps `false`; PartySheet sets it `true` on hover. Old saves without `seen` default to `true`. `rarity: int` drives border + name color in PartySheet and EndCombatScreen — old saves without `rarity` default to COMMON (0) via `.get("rarity", 0)`. Nothing is auto-assigned on pickup. Cleared by `reset()`. **Saved to disk.** |
...
| `vendor_stocks` | `Dictionary` | `{}` | Pre-rolled vendor stock manifests. Keyed by instance_key: CITY vendors → `vendor_id` (e.g. `"vendor_weapon"`); WORLD vendors → map `node_id` (e.g. `"node_o3"`). Each value is an Array of `{ vendor_id, item, price, sold }` entries. Populated once by `MapManager._generate_vendor_stocks()` after map-gen; never re-rolled mid-run (save-scum prevention). Old saves without the key default to `{}` on load, then `_generate_vendor_stocks()` populates them on the next map load. **Saved to disk** under key `"vendor_stocks"`. Reset to `{}` by `reset()`. |
| `encountered_archetypes` | `Array[String]` | `[]` | Archetype ids seen in combat this run. Populated by `record_archetype()`. Drives the Archetypes Log "Encountered" status. **Saved to disk** under key `"encountered_archetypes"`. Old saves default to `[]`. Reset by `reset()`. |
| `recruited_archetypes` | `Array[String]` | `[]` | Archetype ids ever added to the bench (via combat recruit, event recruit, or city hire). Persists even if the follower is later released from the bench. Drives the Archetypes Log "Follower" status. **Saved to disk** under key `"recruited_archetypes"`. Old saves default to `[]`. Reset by `reset()`. |
...
| `add_to_bench` | `(follower: CombatantData) -> bool` | Appends `follower` to `bench` if `bench.size() < BENCH_CAP`. Also calls `record_recruited_archetype(follower.archetype_id)` — **this is the single hookpoint for all follower insertion paths** (combat recruit, event recruit, city hire). Returns `true` on success, `false` if bench is full. Does **not** call `save()` — caller must save explicitly after successful insert. |
| `release_from_bench` | `(index: int) -> void` | Removes `bench[index]`. Auto-deequips all three gear slots (weapon/armor/accessory) back to `GameState.inventory` before removing — each pushed dict includes `rarity: eq.rarity`. Calls `save()`. No-op on out-of-bounds index. |
...
| `record_archetype` | `(id: String) -> void` | Appends `id` to `encountered_archetypes` if non-empty and not already present. Calls `save()`. Called by `CombatManager3D._setup_units()` for each enemy at combat start (encounter recorded even if the player flees). |
| `record_recruited_archetype` | `(id: String) -> void` | Appends `id` to `recruited_archetypes` if non-empty and not already present. Does **not** call `save()` — `add_to_bench()` is the canonical caller and the top-level path always saves after bench insert. |
...
| `regen_world_vendor_stocks` | `() -> void` | Regenerates ONLY the WORLD vendor stocks (entries keyed by map node_ids where `node_types[id] == "VENDOR"`). CITY entries are left untouched. Re-seeds each world node using `hash(str(map_seed) + "::" + node_id)`. Calls `save()`. Future trigger: every 3 boss wins (not yet wired). |
| `reset` | `() -> void` | Resets all in-memory fields to fresh-run defaults (`player_node_id = "badurga"`, `visited_nodes = ["badurga"]`, `map_seed = 0`, `node_types = {}`, `pending_node_type = ""`, `current_combat_node_id = ""`, `cleared_nodes = []`, `threat_level = 0.0`, `used_event_ids = []`, `party = []`, `bench = []`, `run_summary = {}`, `inventory = []`, `gold = 0`, `encountered_archetypes = []`, `recruited_archetypes = []`). Must be called alongside `delete_save()` when wiping a save mid-session. |
...
| `recruited_archetypes` | `GameState.recruited_archetypes` | JSON Array — typed back via `Array(raw, TYPE_STRING, "", null)`; defaults to `[]` on old saves |
| `party` | `GameState.party` | JSON Array of dicts — each dict holds all scalar fields + `abilities`/`ability_pool` as string arrays + `feat_ids` as string array + `level`/`xp`/`pending_level_ups` as ints + `physical_armor`/`magic_armor` as ints + `weapon_id`/`armor_id`/`accessory_id` as strings + `temperament_id` as string + **`spd` as int**. Deserialized back to `Array[CombatantData]` by `_deserialize_combatant()`. **Migrations:** old saves with `armor_defense` key → both armor lanes. Missing `level`/`xp`/`pending_level_ups` default to `1`/`0`/`0`. Missing `temperament_id` defaults to `"even"` (neutral). Missing `spd` defaults to `4`. |
...
| `inventory` | `GameState.inventory` | JSON Array of reward dicts `{id, name, description, item_type, seen, rarity}`. Stored and loaded as-is. `seen` old-save default: `true`. `rarity` old-save default: `0` (COMMON) via `.get("rarity", 0)`. |
| `gold` | `GameState.gold` | `int`. Read back as `int(parsed.get("gold", 0))` — old saves without the key default to `0`. |
| `vendor_stocks` | `GameState.vendor_stocks` | JSON Object keyed by instance_key → Array of stock entry dicts `{ vendor_id, item, price, sold }`. Restored as-is; each entry dict is appended to a typed Array per key. Old saves without the key default to `{}` (empty dict); `MapManager._generate_vendor_stocks()` re-populates on next map load. |
...
| Date | Change |
|---|---|
| 2026-05-02 | **Combat Pivot Slice 1 — `spd` serialization.** `_serialize_combatant()` now writes `"spd": d.spd`. `_deserialize_combatant()` reads `dict.get("spd", 4)` — old saves without the field default to `4`. No other changes to `save()` / `load_save()`. |
| 2026-04-30 | **Vendor Slice 4 — `vendor_stocks` + `regen_world_vendor_stocks()`.** `vendor_stocks: Dictionary = {}` added; keyed by CITY vendor_id or WORLD node_id; each value is Array of `{ vendor_id, item, price, sold }`. Wired into `save()` (key `"vendor_stocks"`), `load_save()` (defaults to `{}` on old saves; restored per-key with dict guard), and `reset()`. `regen_world_vendor_stocks()` regenerates WORLD entries (iterates `node_types` for "VENDOR") without touching CITY; calls `save()`. |
