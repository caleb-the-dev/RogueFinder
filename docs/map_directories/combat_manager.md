...
| `_setup_test_room_units()` | Dispatcher — picks a scenario from `GameState.test_room_kind` and forwards to `_spawn_test_room()`. `"armor_mod"` → armor mod; `"recruit_test"` → recruit test; `"ai_roles"` → AI Roles scenario; `"ai_crit_heal"` → AI Crit-Heal scenario; default → armor showcase. |
| `_spawn_test_room(player_defs, enemy_defs)` | Generic 3v3 spawner — places combatants on the same fixed positions used by both scenarios, calls `unit.setup()`, registers `_attr_snapshots` for player units (including armor mod fields), connects `unit_died`. |
...
| `_process_enemy_actions()` | Full enemy AI loop: **hazard damage check** → movement-heuristic target (random hostile for greedy stride) → consumable use → stride → `EnemyAI.choose_action()` → if null skip; else set `enemy.last_ability_id`, spend energy, execute. 0.65s delay between enemies. |
