...
> Last updated: 2026-04-28 (Follower Slice 4 — recruit success path: board removal, rename prompt, bench insert, bench-full modal; recruit_test room)
...
| `_setup_test_room_units()` | Dispatcher — picks a scenario from `GameState.test_room_kind` and forwards to `_spawn_test_room()`. `"armor_mod"` → armor mod defs; `"recruit_test"` → recruit test defs; default → armor showcase defs. |
