...
> Last updated: 2026-05-02 (Combat Pivot Slice 5 — lane-aware resolve_targets; abilities.csv migrated; _stub_pick_target removed)
...
| `_fire_unit_turn(u)` | **Stub AI (Slice 4–5):** always picks slot 0. Looks up ability via `AbilityLibrary`; calls `resolve_targets()`; iterates over the returned target array calling `_apply_ability()` per target; sets `u.cooldowns[0] = ability.cooldown_max`. If `resolve_targets` returns empty, unit skips turn. Slice 6 replaces stub AI selection with real priority-list AI. |
| `_apply_ability(caster, target, ability)` | Iterates `ability.effects`; handles HARM only: `raw_dmg = base_value + effective_stat(attr)`; subtracts `physical_defense` or `magic_defense` based on `ability.damage_type`; minimum 1 damage. BUFF/DEBUFF/MEND silently ignored until Slice 5+. |
...
- **Stub AI ignores cooldowns** — `_fire_unit_turn()` always picks slot 0 regardless of `u.cooldowns[0]`. Real cooldown gating + role-based AI selection lands in Slice 6.
- **`_apply_ability` is HARM-only** — BUFF abilities (like `bless`) will print but change nothing. Non-HARM effect dispatch lands in a future slice.
- **`end_combat()` does not route to map yet** — it only prints. Slice 6+ will add `change_scene_to_file()` back to the map with reward/defeat handling.
