...
> Last updated: 2026-05-02 (Combat Pivot Slice 6 — AutobattlerEnemyAI wired; _apply_ability extended to HARM/MEND/BUFF/DEBUFF)
...
| `scripts/combat/CombatManagerAuto.gd` | `scenes/combat/CombatSceneAuto.tscn` | **Active** — autobattler tick loop (Slice 6); `class_name CombatManagerAuto extends Node3D`; default combat path |
| `scripts/combat/AutobattlerEnemyAI.gd` | — | **Active** — static module; `class_name AutobattlerEnemyAI extends RefCounted`; role-based priority-list AI for the autobattler |
...
| `end_combat` | `(victory: bool) -> void` | Sets `combat_running = false`; prints result. Slice 6+ will route to `EndCombatScreen` / `RunSummaryScene`. |
| `resolve_targets` *(static)* | `(caster: CombatantData, ability: AbilityData, board: LaneBoard) -> Array[CombatantData]` | Returns all valid targets for the (caster, ability, board) triple. Shape dispatch: `SELF`→caster only; `SAME_LANE`/`SINGLE`→opposite lane with nearest-enemy fallback; `ADJACENT_LANE`→lanes ±1 on enemy side; `ALL_LANES`→every living enemy; `ALL_ALLIES`→every living ally including caster. Legacy shapes (CONE/LINE/RADIAL/ARC) fall back to SAME_LANE logic. Returns empty array when no valid target exists — caller skips turn. |
...
| `_fire_unit_turn(u)` | Calls `AutobattlerEnemyAI.pick(u, allies, hostiles, board)` to get `{ability, targets, slot}`. If `pick.ability == null`, unit skips its turn (logs a "skips" message). Otherwise iterates the returned target array calling `_apply_ability()` per target; sets `u.cooldowns[pick.slot] = ability.cooldown_max`. |
| `_apply_ability(caster, target, ability)` | Iterates `ability.effects`; dispatches via `match effect.effect_type`: HARM → `_apply_harm()`; MEND → heals `base_value + caster.effective_stat("willpower")`, capped at `target.hp_max`; BUFF → `_apply_stat_delta(target, effect, +1)`; DEBUFF → `_apply_stat_delta(target, effect, -1)`; FORCE/TRAVEL → ignored (no movement in autobattler). |
| `_apply_harm(caster, target, effect, attr, dt)` | HARM formula: `raw_dmg = base_value + effective_stat(attr_name)`, `defense` from `physical_defense` or `magic_defense` based on `dt`, final `dmg = max(1, raw_dmg - defense)`. |
| `_apply_stat_delta(target, effect, sign)` | Applies transient stat mutations. For vert slice only `PHYSICAL_ARMOR_MOD` and `MAGIC_ARMOR_MOD` are live (clamp to [-10,10]); all other `target_stat` values are no-ops pending duration tracking in Slice 7. |
...
- **`end_combat()` does not route to map yet** — it only prints. Slice 7 will add `change_scene_to_file()` back to the map with reward/defeat handling.
- **`_apply_stat_delta` is mostly a no-op for vert slice** — only `PHYSICAL_ARMOR_MOD` and `MAGIC_ARMOR_MOD` are wired. All other stat targets (STRENGTH, DEX, etc.) pass silently. Duration tracking lands in Slice 7.
- **BUFF/DEBUFF abilities with non-armor targets currently do nothing** — e.g., `bless` (+1 STR) fires without visible effect because the STRENGTH case is a no-op. This is intentional for the vert slice; the unit still picks correctly via role AI.
- **`resolve_targets` is a static method** — callable as `CombatManagerAuto.resolve_targets(...)` without an instance. Used by `_fire_unit_turn()` and headless tests alike.
- **`ALL_ALLIES` includes the caster** — `resolve_targets` returns every unit on the caster's own side, including the caster. Heal/buff abilities with this shape will self-apply.
...
| Date | Change |
|---|---|
| 2026-05-02 | **Combat Pivot Slice 6 — AutobattlerEnemyAI wired; full effect dispatch.** `AutobattlerEnemyAI.gd` added (see `enemy_ai.md` for module doc). `_fire_unit_turn()` stub deleted; now calls `AutobattlerEnemyAI.pick(u, allies, hostiles, board)`. `_apply_ability()` expanded from HARM-only to full dispatch: HARM delegates to `_apply_harm()` (formula unchanged), MEND heals `base_value + caster.willpower` capped at `hp_max`, BUFF/DEBUFF route to `_apply_stat_delta()` (armor-mod targets live; other stat targets are no-ops pending duration system). Two new helpers extracted: `_apply_harm(caster, target, effect, attr, dt)` and `_apply_stat_delta(target, effect, sign)`. 4 headless tests (`test_autobattler_ai.gd/.tscn`) — ATTACKER prefers HARM, HEALER prefers MEND, cooldown skip, HARM-fallback to BUFF. |
| 2026-05-02 | **Combat Pivot Slice 5 — lane-aware target resolver.** `CombatManagerAuto.resolve_targets(caster, ability, board)` static method added. Dispatches on `ability.target_shape`: `SELF`→caster, `SAME_LANE`/`SINGLE`→opposite-lane with fallback, `ADJACENT_LANE`→lanes ±1, `ALL_LANES`→all enemies, `ALL_ALLIES`→all allies including caster; legacy shapes fall back to SAME_LANE. `_fire_unit_turn` updated: calls `resolve_targets`, iterates over returned array (supports multi-hit AoE), logs `on N target(s)`. `_stub_pick_target` deleted. 6 headless tests added (`test_lane_targeting.gd/.tscn`); `test_combat_loop.gd` updated with `get_tree().quit()`. |
