...
> Last updated: 2026-05-02 (Combat Pivot Slice 1 — SPD added as 6th attribute column)
...
- **TOP-RIGHT:** Derived stats — **P.Def / M.Def / Speed / EN Max / Regen (blue, 5 cols, font_size 13)**. `P.Def` = `physical_defense`; `M.Def` = `magic_defense`; `Regen` = `energy_regen`. No ATK col — there is no attack stat; ability damage scales from the caster's relevant attribute (STR/DEX/COG) directly. Base attributes — **STR / DEX / COG / WIL / VIT / SPD (6 cols)**. SPD = `effective_stat("spd")` = `spd + kindred_spd_bonus + all pillar bonuses`. **Attribute coloring:** yellow = no item bonus; **green** = item/accessory-feat is boosting the stat; **red** = item is penalizing. Only `get_equip_bonus(stat)` + the accessory's feat bonus (if not already in `feat_ids`) contribute to color — background/class feats and temperament do NOT turn attrs green. Hovering an attribute shows "+N from items" in the tooltip when boosted. **Level row** (below attributes): "Lv. X" (13 px, blue-grey, centered in TR width) OR "Level Up! (N)" button when `pending_level_ups > 0` (rainbow modulate tween + scale pulse 1.0↔1.07; draws on top of the level label which is suppressed when pending > 0). All labels: `tooltip_text` + `MOUSE_FILTER_PASS`.
...
|---|---|---|
| 2026-05-02 | Combat Pivot Slice 1 | **SPD added as 6th attribute column.** `attr_defs` array extended from 5 to 6 entries — `["SPD", "spd", member.spd, "Speed\nDrives turn countdown in autobattler. = spd + kindred bonus."]`. Column width `acol_w` auto-adjusts since it divides `tr_w` by `attr_defs.size()`. The SPD column uses the same green/red item-bonus coloring as the other five. `_stat_abbr()` updated to return `"SPD"` for key `"spd"`. SPD is not used by the old 3D combat system — it is a placeholder for the autobattler countdown. |
