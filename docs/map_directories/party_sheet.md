...
> Last updated: 2026-04-29 (Accessory Tier Families Slice 5 — tooltip feat/ability lines; 6-col derived stats; attribute green/red coloring; accessory feat in Feats tab; tab persistence across equip)
...
- **TOP-RIGHT:** Derived stats — **Atk / P.Def / M.Def / Speed / EN Max / Regen (blue, 6 cols, font_size 13)**. `P.Def` = `physical_defense`; `M.Def` = `magic_defense`; `Regen` = `energy_regen`. Base attributes — STR / DEX / COG / WIL / VIT (5 cols). **Attribute coloring:** yellow = no item bonus; **green** = item/accessory-feat is boosting the stat; **red** = item is penalizing. Only `get_equip_bonus(stat)` + the accessory's feat bonus (if not already in `feat_ids`) contribute to color — background/class feats and temperament do NOT turn attrs green. Hovering an attribute shows "+N from items" in the tooltip when boosted. **Level row** (below attributes): "Lv. X" (13 px, blue-grey, centered in TR width) OR "Level Up! (N)" button when `pending_level_ups > 0` (rainbow modulate tween + scale pulse 1.0↔1.07; draws on top of the level label which is suppressed when pending > 0). All labels: `tooltip_text` + `MOUSE_FILTER_PASS`.
...
| 2026-04-29 | Accessory Tier Families (Slice 5) | **Derived stats expanded from 4 to 6 columns.** TOP-RIGHT derived row now shows: **Atk / P.Def / M.Def / Speed / EN Max / Regen** (was Speed/P.Def/M.Def/EN Max). Value font_size reduced 14→13 to fit. Attack and Energy Regen were previously invisible in the member card — now visible and update on equip. |
