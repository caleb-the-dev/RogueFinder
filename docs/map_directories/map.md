...
| Field | Value |
|---|---|
| last_updated | 2026-05-02 (Combat Pivot Slice 4 — CountdownTracker, CombatManagerAuto tick loop, GameState bridge, MapManager enemy rolling, USE_AUTOBATTLER_COMBAT = true) |
...
| [Combat Manager — Autobattler + 3D legacy](combat_manager.md) | `combat_manager.md` | ✅ Active (autobattler — `CombatManagerAuto` tick loop live; `CountdownTracker`; `LaneBoard`; `USE_AUTOBATTLER_COMBAT = true`; stub AI slot-0 + HARM-only; real AI + lane targeting in Slices 5–6) + 🔧 Legacy (3D — `CombatManager3D` still compiles; retired Slice 7) | Core |
