...
| Field | Value |
|---|---|
| last_updated | 2026-05-02 (Combat Pivot Slice 6 — AutobattlerEnemyAI; _apply_ability full dispatch HARM/MEND/BUFF/DEBUFF) |
...
| [Enemy AI](enemy_ai.md) | `enemy_ai.md` | ✅ Active (`AutobattlerEnemyAI` — priority-list pick, role prefs from archetype, cooldown + target filters, Slice 6 live) + 🔧 Legacy (`EnemyAI.gd` for CombatManager3D — Slices 1–3 done; retires Slice 7) | Core |
| [Combat Manager — Autobattler + 3D legacy](combat_manager.md) | `combat_manager.md` | ✅ Active (autobattler — `CombatManagerAuto` + `AutobattlerEnemyAI` + `CountdownTracker` + `LaneBoard`; `USE_AUTOBATTLER_COMBAT = true`; **full ability dispatch HARM/MEND/BUFF/DEBUFF live (Slice 6)**) + 🔧 Legacy (3D — `CombatManager3D` still compiles; retired Slice 7) | Core |
| [Grid System](grid_system.md) | `grid_system.md` | ✅ Active (3D Grid3D) + Legacy (2D) + 🔧 LaneBoard (3-lane × 2-side pure-data structure; coexists; Grid3D survives until Slice 7) | Core |
...
| [Combatant Data Model + ArchetypeLibrary](combatant_data.md) | `combatant_data.md` | ✅ Active (ArchetypeLibrary CSV-sourced S34; speed = 1 + kindred_bonus only — dex severed; no attack stat — effective_stat() drives HARM; **spd: int = 4** added as 6th core attribute — kindred SPD bonuses live, persisted, shown in PartySheet/StatPanel/CharacterCreation) | Data |
| [Ability System (AbilityData / EffectData / AbilityLibrary)](ability_system.md) | `ability_system.md` | ✅ Active (64 abilities; upgraded_id live; cooldown_max live; **lane TargetShape values active (Slice 5)** — `SAME_LANE/ADJACENT_LANE/ALL_LANES/ALL_ALLIES`; all 64 CSV rows migrated; legacy CONE/LINE/RADIAL/ARC retire Slice 7) | Data |
...
| [Kindred Library](combatant_data.md) | `combatant_data.md` | ✅ Active (8 kindreds — Human/Half-Orc/Gnome/Dwarf/Skeleton/Giant Rat/Spider/Dragon; owns ability lane; no longer grants feats; **spd bonus values live** in stat_bonuses: Spider +3, Rat +2, Half-Orc/Dwarf/Skeleton −1, others 0) | Data |
| [Main Menu + Character Creation](character_creation.md) | `character_creation.md` | ✅ Active (deterministic stats; slot 0 = class ability, slot 1 = kindred ability; bg feat seeds feat_ids; **SPD preview label live**; 12 tests) | Presentation |
...
```
CombatManagerAuto  (active — Slice 6 full AI + effect dispatch; default combat path)
  ├── AutobattlerEnemyAI    (static module — priority-list pick per role; Slice 6)
  ├── LaneBoard             (3-lane × 2-side pure-data structure; owns placement)
  ├── CountdownTracker      (static module — all countdown + cooldown math)
  ├── AbilityLibrary        (get_ability() to resolve abilities each turn)
  ├── ArchetypeLibrary      (enemy creation via MapManager._roll_combat_enemies; role lookup in AI)
  └── GameState             (party + pending_combat_enemies bridge fields)
...
│   ├── combat/
│   │   ├── CombatManager3D.gd          ← active (3D tactical grid; legacy — retires Slice 7)
│   │   ├── CombatManagerAuto.gd        ← autobattler (Slice 6 — full AI + effect dispatch); class_name CombatManagerAuto; DEFAULT combat path
│   │   ├── AutobattlerEnemyAI.gd       ← static module; class_name AutobattlerEnemyAI extends RefCounted; priority-list pick for autobattler (Slice 6)
│   │   ├── CountdownTracker.gd         ← static module; class_name CountdownTracker extends RefCounted; countdown + cooldown math
│   │   ├── LaneBoard.gd                ← 3-lane × 2-side data structure; class_name LaneBoard extends RefCounted
│   │   ├── EnemyAI.gd                  ← static module; choose_action() + role picker; for CombatManager3D only; retires Slice 7
│   │   ├── Unit3D.gd                   ← active
│   │   ├── Grid3D.gd                   ← active
│   │   ├── QTEBar.gd
...
│       ├── PlacementOverlay.gd         ← pre-fight lane assignment skeleton (Slice 3); class_name PlacementOverlay; layer 22; emits placement_locked
...
├── data/
│   ├── abilities.csv                   ← 64 abilities; effects as JSON arrays; upgraded_id live; cooldown column added (Slice 2); **target column migrated to lane shapes (Slice 5)** — SAME_LANE/ADJACENT_LANE/ALL_LANES/ALL_ALLIES; read via res://data/
...
│   ├── combat/
│   │   ├── CombatScene3D.tscn          ← active (3D tactical)
│   │   ├── CombatSceneAuto.tscn        ← autobattler (active — Slice 4 tick loop); minimal (root Node3D + CombatManagerAuto script)
...
│   ├── ui/
│   │   └── PlacementOverlay.tscn       ← minimal (root CanvasLayer + PlacementOverlay script); visible=false by default
│   └── ui/
...
| Date | Area | Note |
|---|---|---|
| 2026-05-02 | AutobattlerEnemyAI, CombatManagerAuto | **Combat Pivot Slice 6 — AI module + full effect dispatch.** `AutobattlerEnemyAI.gd` added (`class_name AutobattlerEnemyAI extends RefCounted`): `ROLE_PREFS` table maps `ArchetypeData.Role` enum → `[EffectType...]`; `pick(unit, allies, hostiles, board)` walks prefs → available (off-cooldown) slots → `_has_effect_type()` check → `resolve_targets()` non-empty → return `{ability, targets, slot}`. `_get_role_prefs()` reads `unit.archetype_id` via `ArchetypeLibrary`; falls back to DEFAULT_PREFS (HARM-first) for generic/empty IDs. `CombatManagerAuto._fire_unit_turn()` stub deleted; replaced with `AutobattlerEnemyAI.pick()` — units with no valid action skip their turn. `_apply_ability()` expanded from HARM-only to full dispatch: HARM → `_apply_harm()` (formula unchanged), MEND → heals `base_value + willpower` capped at `hp_max`, BUFF/DEBUFF → `_apply_stat_delta()` (armor-mod targets live; other stat targets no-op pending Slice 7). 4 headless tests — all pass. |
| 2026-05-02 | AbilityData, AbilityLibrary, abilities.csv, CombatManagerAuto | **Combat Pivot Slice 5 — lane targeting.** `AbilityData.TargetShape` gained `SAME_LANE(6)`, `ADJACENT_LANE(7)`, `ALL_LANES(8)`, `ALL_ALLIES(9)`. `AbilityLibrary._TARGET_SHAPE` dict extended. All 64 `abilities.csv` rows migrated (SAME_LANE: SINGLE→; ALL_LANES: RADIAL ANY/ENEMY + ARC HARM; ALL_ALLIES: RADIAL ALLY + ARC ALLY + SINGLE ALLY; ADJACENT_LANE: CONE + ARC DEBUFF). `CombatManagerAuto.resolve_targets(caster, ability, board)` static method added — shape dispatch via match, empty array on no target. `_stub_pick_target` deleted. `_fire_unit_turn` iterates resolved target array (multi-hit AoE supported). 6 tests (`test_lane_targeting`). |
| 2026-05-02 | CountdownTracker, CombatManagerAuto, CombatantData, GameState, MapManager | **Combat Pivot Slice 4 — countdown engine + tick loop.** `CountdownTracker.gd` added (static module — `compute_countdown_max`, `tick`, `tick_and_collect_ready`, `tick_cooldowns`, `available_slot_indices`, `tiebreak_ready`, `reset_countdown`; formula `clamp(8-spd,2,12)`). `CombatantData` gains 3 transient fields: `countdown_current`, `countdown_max`, `cooldowns: Array[int] = [0,0,0]` (NOT serialized). `CombatManagerAuto` fully rewritten: `start_combat()` places units + seeds countdown/cooldowns; `_advance_tick()` drives tick loop with tiebreak ordering; stub AI always picks slot 0; `_apply_ability()` HARM-only; `_process()` 0.6s real-time driver. `GameState.pending_combat_enemies: Array[CombatantData] = []` transient bridge (NOT serialized). `MapManager._roll_combat_enemies()` helper + COMBAT/BOSS branch fills the bridge before scene load. `USE_AUTOBATTLER_COMBAT` flipped to `true` — autobattler is the default. 6 tests (`test_countdown_tracker`) + 1 integration test (`test_combat_loop`) — all pass. Note: stub AI fires slot 0 regardless of ability type; BUFF abilities deal 0 damage (expected until Slice 5). |
| 2026-05-02 | LaneBoard, CombatManagerAuto, PlacementOverlay, MapManager | **Combat Pivot Slice 3 — combat scaffold.** `LaneBoard.gd` (3 lanes × 2 sides, pure-data, `class_name LaneBoard extends RefCounted`): `place/remove/get_unit/get_opposite/get_adjacent_lane_units/get_all_on_side/is_side_wiped`. `CombatManagerAuto.gd` (`class_name CombatManagerAuto extends Node3D`): skeleton with `board: LaneBoard`, `start_combat()`, `end_combat()` — no tick loop yet. `CombatSceneAuto.tscn`: minimal (root Node3D + CombatManagerAuto script). `PlacementOverlay.gd` (`class_name PlacementOverlay extends CanvasLayer`, layer 22): `show_placement(party)`, emits `placement_locked`. `MapManager.USE_AUTOBATTLER_COMBAT: bool = false` — feature flag; COMBAT/BOSS dispatch gates on it (false=old combat, true=CombatSceneAuto). 6 headless tests (`test_lane_board.gd/.tscn`). Note: `CombatManagerAuto.gd` is in `scripts/combat/` (not `CombatManager.gd` — that name is taken by the legacy 2D script). |
| 2026-05-02 | CombatantData, KindredLibrary (kindreds.csv), GameState, PartySheet, StatPanel, CharacterCreationManager | **Combat Pivot Slice 1 — SPD attribute foundation.** `CombatantData.spd: int = 4` added as the 6th core attribute (`@export_range(1,10)`). `effective_stat("spd")` handles the full 7-source stack. `kindreds.csv` `stat_bonuses` extended with `spd:X` for all 8 kindreds (Spider +3, Giant Rat +2, Human/Gnome/Dragon 0, Half-Orc/Dwarf/Skeleton −1). `GameState._serialize_combatant()` writes `spd`; `_deserialize_combatant()` reads it with old-save default 4. **PartySheet:** `attr_defs` expanded from 5 to 6 cols (SPD added, uses same green/red item-bonus coloring). **StatPanel:** SPD appended to WIL/VIT line. **CharacterCreationManager:** `_preview_spd_lbl` label added; `_calc_preview()` computes and pushes SPD. **5 headless tests** (`test_spd_attribute.gd`) — all pass. Old combat system unaffected (spd unused there). |
| 2026-05-02 | AbilityData, AbilityLibrary, abilities.csv | **Combat Pivot Slice 2 — cooldown field (additive).** `AbilityData` gained `@export var cooldown_max: int = 0` (legacy comment on `energy_cost`). `abilities.csv` gained `cooldown` column after `cost` — all 63 rows populated per migration table (cost 0→0, 1–2→2, 3–4→3, 5+→5). `AbilityLibrary._row_to_data()` gained `"cooldown"` case → `a.cooldown_max`. Old combat unaffected (`energy_cost` untouched). 3 headless tests. |
