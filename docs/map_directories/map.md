...
| Field | Value |
|---|---|
| last_updated | 2026-05-02 (Combat Pivot Slice 5 — lane TargetShape enum; abilities.csv migrated; resolve_targets static method; _stub_pick_target removed) |
...
| [Combat Manager — Autobattler + 3D legacy](combat_manager.md) | `combat_manager.md` | ✅ Active (autobattler — `CombatManagerAuto` tick loop live; `CountdownTracker`; `LaneBoard`; `USE_AUTOBATTLER_COMBAT = true`; stub AI slot-0; **`resolve_targets` shape-aware targeting live (Slice 5)**; real AI in Slice 6) + 🔧 Legacy (3D — `CombatManager3D` still compiles; retired Slice 7) | Core |
...
```
CombatManagerAuto  (active — Slice 4 tick loop; default combat path)
  ├── LaneBoard             (3-lane × 2-side pure-data structure; owns placement)
  ├── CountdownTracker      (static module — all countdown + cooldown math)
  ├── AbilityLibrary        (get_ability() to resolve slot 0 on each turn)
  ├── ArchetypeLibrary      (enemy creation via MapManager._roll_combat_enemies)
...
│   ├── combat/
│   │   ├── CombatManager3D.gd          ← active (3D tactical grid)
│   │   ├── CombatManagerAuto.gd        ← autobattler (Slice 4 — tick loop live); class_name CombatManagerAuto; DEFAULT combat path
