...
| Field | Value |
|---|---|
| last_updated | 2026-05-02 (Combat Pivot Slice 3 — LaneBoard, CombatManagerAuto, PlacementOverlay, CombatSceneAuto, USE_AUTOBATTLER_COMBAT flag) |
...
| [Combat Manager — 3D legacy + Autobattler skeleton](combat_manager.md) | `combat_manager.md` | ✅ Active (3D) + 🔧 Skeleton (autobattler — `CombatManagerAuto`; `LaneBoard`; `PlacementOverlay`; `CombatSceneAuto.tscn`; `USE_AUTOBATTLER_COMBAT = false` in MapManager keeps old combat default) | Core |
...
```
CombatManagerAuto  (skeleton — Slice 3; no real deps yet)
...
│   │   ├── CombatManagerAuto.gd        ← autobattler skeleton (Slice 3); class_name CombatManagerAuto; no tick loop yet
...
│   │   ├── CombatSceneAuto.tscn        ← autobattler skeleton (Slice 3); minimal (root Node3D + CombatManagerAuto script)
