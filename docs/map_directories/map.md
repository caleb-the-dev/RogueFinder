...
| Field | Value |
|---|---|
| last_updated | 2026-04-28 (Pause Menu + Archetypes Log: PauseMenuManager layer 26, SettingsStore, encountered_archetypes, record_archetype) |
| last_groomed | 2026-04-25 |
| sessions_since_groom | 17 |
| groom_trigger | 10 |
...
| [Pause Menu + Settings + Archetypes Log](hud_system.md) | `hud_system.md` | ✅ Active — CanvasLayer layer 26 autoload; ESC toggle; Settings (fullscreen + vol sliders); Guide stub; Archetypes Log (encountered_archetypes driven Pokédex panel) | Presentation |
| [SettingsStore](hud_system.md) | `hud_system.md` | ✅ Active — autoload; user://settings.json; fullscreen + volume prefs | Global |
...
PauseMenu (autoload, CanvasLayer layer 26)
  ├── GameState             (encountered_archetypes for Archetypes Log)
  ├── ArchetypeLibrary      (get_archetype for log card display)
  └── SettingsStore         (read/write fullscreen + volume on settings panel)
...
│   │   ├── SettingsStore.gd            ← autoload; user://settings.json; fullscreen + volume prefs
...
| Date | Area | Note |
|---|---|---|
| 2026-04-28 | PauseMenuManager (new), SettingsStore (new), GameState, CombatManager3D, ArchetypeData, ArchetypeLibrary, project.godot, tests | **Pause Menu + Archetypes Log.** `PauseMenuManager.gd` added as CanvasLayer autoload (layer 26, `PROCESS_MODE_WHEN_PAUSED`). ESC toggles open/closed from MapScene/CombatScene3D/BadurgaScene/CharacterCreationScene; blocked from MainMenuScene and RunSummaryScene via `_scene_name_is_pauseable()`. Sub-panels: Resume/Settings/Guide/Archetypes Log/Main Menu/Exit. Settings: fullscreen toggle (calls `DisplayServer.window_set_mode`) + Master/Music/SFX volume sliders (placeholder; no audio bus). Guide: stub "coming soon" panel. Archetypes Log: scrollable Pokédex-style cards built from `GameState.encountered_archetypes`; RogueFinder excluded from display; empty-state text. `SettingsStore.gd` autoload added — `user://settings.json` separate from run save. `GameState.encountered_archetypes: Array[String]` + `record_archetype(id)` added with full save/load/reset wiring (typed-array pattern). `CombatManager3D._setup_units()`: `GameState.record_archetype(cd.archetype_id)` called for each enemy at combat start. CM3D ESC handler fixed: only consumes the event when it actually deselected/cancelled something (IDLE + nothing selected → falls through). `ArchetypeData.notes: String` field added; `ArchetypeLibrary` now parses `notes` column. 8 headless tests (`test_pause_menu.gd`). |
