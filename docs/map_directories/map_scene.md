...
**`_build_dev_event_panel()`** — creates a `CanvasLayer` (layer 30) with a full-screen dark overlay and a centered `PanelContainer`. Inside (top to bottom): title + close; **XP/LEVEL section** ("Grant XP +20 (all)" + "Force Level-Up (all)", both call `_refresh_party_btn()`); separator; **COMBAT section** with "⚔ Test Room (armor showcase)" button — sets `GameState.test_room_mode = true`, hides panel, calls `change_scene_to_file("res://scenes/combat/CombatScene3D.tscn")` (spawns hardcoded 3v3, no save mutation, returns to map on exit); separator; **EVENTS section** with a `ScrollContainer` containing one `Button` per event from `EventLibrary.all_events()`.
