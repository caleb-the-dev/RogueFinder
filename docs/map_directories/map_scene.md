...
**`_build_dev_event_panel()`** — creates a `CanvasLayer` (layer 30) with a full-screen dark overlay and a centered `PanelContainer`. Inside (top to bottom): title + close; **XP/LEVEL section** ("Grant XP +20 (all)" + "Force Level-Up (all)", both call `_refresh_party_btn()`); separator; **COMBAT section** with two side-by-side buttons — "⚔ Test Room — Armor Showcase" sets `GameState.test_room_kind = "armor_showcase"`, "⚔ Test Room — Armor Mod" sets `test_room_kind = "armor_mod"`. Both flip `test_room_mode = true`, hide the panel, and call `change_scene_to_file("res://scenes/combat/CombatScene3D.tscn")` — `_end_combat()` returns to the map without saving or granting XP. Separator; **INVENTORY section** with "+ Add Item to Inventory" button — hides this panel and calls `_show_add_item_panel()` (see below). Separator; **EVENTS section** with a `ScrollContainer` containing one `Button` per event from `EventLibrary.all_events()`.
