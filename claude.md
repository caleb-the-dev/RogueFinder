...
- **Last session (Combat Pivot Slice 4, 2026-05-02):** `CountdownTracker` static module (compute_countdown_max, tick, tick_and_collect_ready, tick_cooldowns, available_slot_indices, tiebreak_ready, reset_countdown — 6 headless tests). `CombatantData` gains 3 transient autobattler fields (countdown_current, countdown_max, cooldowns). `CombatManagerAuto` fully implemented: `start_combat()`, `_advance_tick()`, stub AI (always slot 0), `_stub_pick_target()`, `_apply_ability()` (HARM-only stub), `_process()` real-time tick driver (0.6s). `GameState.pending_combat_enemies` bridge field. `MapManager._roll_combat_enemies()` helper + COMBAT/BOSS path stashes enemies before loading `CombatSceneAuto.tscn`. `USE_AUTOBATTLER_COMBAT = true` — autobattler is now the default path. 2 new test files (test_countdown_tracker, test_combat_loop). 9 total Slice 4 tests pass.
- **Previous session (Combat Pivot Slice 3, 2026-05-02):** `LaneBoard` (3-lane × 2-side pure-data, 6 tests), `CombatManagerAuto` skeleton, `PlacementOverlay` skeleton, `CombatSceneAuto.tscn`, `USE_AUTOBATTLER_COMBAT` flag added.
- **Deferred:** Character creation B3 (scrapped); portrait serialization (art pass); Badurga Tavern + Bulletin Board (coming-soon stubs); ability effects placeholder; boss difficulty scaling (Feature 8); trigger-based feat effects; feat rarity/tiering/swapping; enemy qte_resolution retune; level-up stat bonuses; audio bus wiring; regen_world_vendor_stocks() trigger not wired; sell-back/services/ability-buying/faction-rep (deferred vendor features); **Enemy AI Slice 4 — FORCE multi-step positioning planner** (stride-to-align for push/pull through hazards; feature prompt written); **Camera Overhaul** (camera stays post-QTE; follows active enemy during enemy turn; player override via RMB/directional; feature prompt written); **Combat Pivot Slices 5–8** (lane targeting → AI module → cutover → consumable balance)
