...
- **Last session (events Slice 6 — full event pass + MapManager polish, 2026-04-25):** Authored 12 new events in CSVs (3 rings covered). Post-testing revision pass: `wounded_traveler` → Wandering Medic; `survivor_in_the_dark` removed; `stray_dog` → recruit placeholder; road_patrol/river_crossing/farmhouse mechanics adjusted; mass_grave and ember_idol choices pruned. Final: events.csv 15 rows, event_choices.csv 46 rows. `MapManager.gd`: dev event test panel (CanvasLayer layer 30, "Events [DEV]" button, `_is_dev_event` flag, fires events without clearing nodes); `_refresh_threat_meter()` for live bar updates (called on traversal + event finish); CITY nodes excluded from ✗ cleared stamp.
- **Deferred:** Character creation B3 (Dial widget component — scrapped, not needed for vert slice); portrait serialization (deferred to art pass); Badurga section content (all 6 buttons are stubs); Vendor scene content (NodeStub placeholder); per-ability QTE styles; ability effects are placeholder; boss difficulty scaling from threat quadrants (Feature 8); kindred feat mechanical effects (all feats named but have no gameplay effect yet); recruit mechanic (stray_dog + mercenary_camp choices are placeholders — party slot system not yet live)
