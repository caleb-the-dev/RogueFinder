...
- **Last session (events Slice 5 — player_pick picker + new-item glow, 2026-04-25):** `EventManager.gd` — `target_picked` signal; `_on_choice_pressed` async coroutine with picker flow; picker overlay (`CenterContainer` + `PanelContainer`, ~500×200 px, alive-member buttons, freed after pick); `dispatch_effect` extended with `forced_target: CombatantData = null`; `_resolve_with_override` helper. `GameState.gd` — `add_to_inventory()` stamps `seen = false`. `PartySheet.gd` — unseen items: gold border + looping alpha tween; `mouse_entered` clears glow via `seen = true` + `_rebuild()`. 7 new headless tests (96 total).
- **Deferred:** Character creation B3 (Dial widget component — scrapped, not needed for vert slice); portrait serialization (deferred to art pass); Badurga section content (all 6 buttons are stubs); Vendor scene content (NodeStub placeholder); per-ability QTE styles; ability effects are placeholder; boss difficulty scaling from threat quadrants (Feature 8); kindred feat mechanical effects (all feats named but have no gameplay effect yet); event authoring pass — 13+ real events (Slice 6)
