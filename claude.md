...
- **Last session (UX Polish, 2026-05-02):** ESC backs out of level-up pick overlay without consuming pick. Auto-slot: picked ability fills first empty abilities slot. Ability glow: new_ability_ids transient field on CombatantData; gold pulse in Abilities tab cleared on hover. All CSV descriptions (abilities/feats/equipment/consumables) rewritten to purely functional — no flavor text.
- **Previous session (Enemy AI Slice 3 — Within-Bucket Scoring, 2026-05-01):** Per-type scorers (HARM/MEND/BUFF/DEBUFF). FORCE disabled. Move priority sort. pick_stride_target(). Buff/debuff tracker on Unit3D. _spawn_test_room typed-array fix. 6 new test rooms. 13 headless tests.
