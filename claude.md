...
- **SPD attribute (Combat Pivot Slice 1, 2026-05-02):** `CombatantData.spd: int = 4` — 6th core attribute. `effective_stat("spd")` wired (full 7-source stack). kindreds.csv updated with spd bonuses (Spider +3, Rat +2, Half-Orc/Dwarf/Skeleton −1, others 0). Serialized (old saves default 4). PartySheet 6th column. StatPanel WIL/VIT/SPD line. CharacterCreation preview SPD label. 5 headless tests. **Not used by old 3D combat — reserved for autobattler countdown (Slice 3+).**
- **Cooldown field (Combat Pivot Slice 2, 2026-05-02):** `AbilityData.cooldown_max: int = 0` added. `abilities.csv` gains `cooldown` column — all 63 rows populated (cost 1–2→2, 3–4→3, 5+→5). `AbilityLibrary` parses `"cooldown"` → `cooldown_max`. `energy_cost` untouched; both fields coexist until Slice 7 cutover. 3 headless tests.
- **Last session (Combat Pivot Slice 6, 2026-05-02):** `AutobattlerEnemyAI.gd` added — static module; `ROLE_PREFS` table maps `ArchetypeData.Role` enum → `[EffectType...]`; `pick(unit, allies, hostiles, board)` returns `{ability, targets, slot}` using role prefs + cooldown filter + valid-target filter. `CombatManagerAuto._fire_unit_turn()` stub replaced with `AutobattlerEnemyAI.pick()`. `_apply_ability()` extended: HARM → `_apply_harm()`, MEND → heals `base_value + willpower` capped at `hp_max`, BUFF/DEBUFF → `_apply_stat_delta()` (armor-mod targets live; other stat targets no-op pending Slice 7). 4 headless tests pass.
- **Previous session (Combat Pivot Slice 5, 2026-05-02):** `AbilityData.TargetShape` extended with `SAME_LANE(6)`, `ADJACENT_LANE(7)`, `ALL_LANES(8)`, `ALL_ALLIES(9)`. `AbilityLibrary._TARGET_SHAPE` dict extended. All 64 `abilities.csv` rows migrated to lane shapes. `CombatManagerAuto.resolve_targets(caster, ability, board)` static method — shape dispatch, empty array on no target. `_stub_pick_target` deleted; `_fire_unit_turn` iterates resolved target array. 6 new tests (`test_lane_targeting`). `test_combat_loop` updated with `get_tree().quit()`. Live: `Smoke Bomb on 3 target(s)` (ALL_LANES), `Slipshot on 1 target(s)` (SAME_LANE), combat resolves correctly.
- **Deferred:** Character creation B3 (scrapped); portrait serialization (art pass); Badurga Tavern + Bulletin Board (coming-soon stubs); boss difficulty scaling (Feature 8); trigger-based feat effects; feat rarity/tiering/swapping; enemy qte_resolution retune; level-up stat bonuses; audio bus wiring; regen_world_vendor_stocks() trigger not wired; sell-back/services/ability-buying/faction-rep (deferred vendor features); **Enemy AI Slice 4 — FORCE multi-step positioning planner** (feature prompt written); **Camera Overhaul** (feature prompt written); **Combat Pivot Slices 7–8** (cutover → consumable balance)
