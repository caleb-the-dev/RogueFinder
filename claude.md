...
- **Last session (Armor Mod, 2026-04-27):** **Part 1 — runtime armor lane.** `AbilityData.Attribute` enum extended with `PHYSICAL_ARMOR_MOD = 6` and `MAGIC_ARMOR_MOD = 7`. `AbilityLibrary._ATTRIBUTE` lookup gained matching string keys so JSON `"stat":"PHYSICAL_ARMOR_MOD"` resolves correctly in effects cells. `CombatantData` gained two transient `int` fields — `physical_armor_mod` + `magic_armor_mod`, both default 0, plain `var` (NOT `@export`/serialized). `physical_defense` and `magic_defense` formulas extended with the mod field as the sixth source. `CombatManager3D._apply_stat_delta` gained the two new cases (clamp `[-10, 10]`); `STAT_STATUS_NAMES` + `STAT_ABBREV` extended with `P.Hardened`/`P.Cracked`/`P.ARM` and `M.Warded`/`M.Exposed`/`M.ARM`. `_attr_snapshots` build sites in both `_setup_units()` and `_setup_test_room_units()` now record the mod fields; `_end_combat()` restores them via `snap.get(..., 0)`. `stone_guard` (Dwarf kindred ancestry) and `divine_ward` (Warden pool) CSV rows updated from the dead `"ARMOR_DEFENSE"` `target_stat` to the new enum names — both abilities now mechanically active. **Part 2 — second test room + new armor + dev tools.** `GameState.test_room_kind: String = "armor_showcase"` transient flag added so multiple test scenarios can share `test_room_mode`; reset to default by `_end_combat`. `CombatManager3D._setup_test_room_units` refactored into a dispatcher; common spawn flow factored to `_spawn_test_room(player_defs, enemy_defs)`; per-scenario data lives in `_armor_showcase_*_defs()` and `_armor_mod_*_defs()` getters. New `armor_mod` scenario: Boran (Dwarf vanguard, owns `stone_guard`) / Velis (Human warden, owns `divine_ward`) / Rune (Gnome arcanist) vs Stone Bruiser / Pyromancer / Twin Threat (all 4/4 armor). `equipment.csv` gained `plate_cuirass` (ARMOR — physical_armor +3, dexterity -2) and `warded_robe` (ARMOR — magic_armor +2 — first equipment piece exercising `_equip_bonus("magic_armor")`). `MapManager` dev menu: existing test-room button renamed "Armor Showcase"; new sibling "Armor Mod" added; new INVENTORY section with "+ Add Item to Inventory" → modal `CanvasLayer` listing every equipment + consumable A-Z, click to drop into bag (rebuilds list on every open so CSV edits hot-reload). `_input()` early-out gate extended for the new panel. 11 new armor-mod tests + 4 new equipment tests; all prior suites still green.
