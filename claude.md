...
- **Last session (Vendor Slice 4 — Stock Manifest + Persistence, 2026-04-30):** `StockGenerator.gd` (new — `roll_stock(vendor, seed_int)` seeded Fisher-Yates + `PricingFormula.price_for()` per entry; returns `[{vendor_id, item, price, sold:false}]`). `RewardGenerator._eq_to_dict/_con_to_dict` promoted to public `eq_to_dict/con_to_dict`. `GameState.vendor_stocks: Dictionary = {}` (CITY keyed by vendor_id, WORLD by node_id; `save()`/`load_save()`/`reset()` wired; `regen_world_vendor_stocks()` added). `MapManager._generate_vendor_stocks()` called after `_assign_boss_type()` — no-op on loaded saves, handles old-save migration. 7 headless tests (`test_vendor_stock.gd/.tscn`). Fixed 6 pre-existing GDScript warnings.
- **Previous session (Vendor Slice 3 — VendorLibrary Data Set, 2026-04-30):** `VendorData.gd` resource (vendor_id, display_name, flavor, category_pool: Array[String], stock_count: int, scope: String). `vendors.csv` — 7 seed rows: 4 CITY specialists (weapon/armor/accessory/consumable) + 3 WORLD roamers (road_peddler, wandering_quartermaster, apothecary_caravan). `VendorLibrary.gd` — BackgroundLibrary shape exactly; `get_vendor()` never-null stub (id="unknown"); `all_vendors()`; `vendors_by_scope(scope)` for Slice 4; `reload()`. `economy.md` bucket created; `map.md` updated. 17 headless tests (`test_vendor_library.gd/.tscn`).
