...
- **Live systems:** 3D combat loop · traversable world map with 5 node types (COMBAT, VENDOR, EVENT, BOSS, CITY) + structured placement rules · save/load · reward system · Badurga city shell (placeholder sections) · threat escalation counter + HUD bar · persistent party (HP/energy carry-over, ally permadeath, run-end summary screen) · party bag inventory (all reward items land in shared bag; equipment + consumables stored as raw dicts) · party sheet overlay (interactive 4-quadrant equip UI, layer 20, full ability pool swap with drag-compare) · CombatActionPanel (right slide-in, player + enemy view, tooltips) · **Kindreds** (species/ancestry + mechanical bonuses — speed/HP driven by KindredLibrary; placeholder feats assigned per kindred; shown in StatPanel/CombatActionPanel/PartySheet) · **MainMenuScene** (title screen; Continue grayed when no save exists; Start New Run wipes state) · **Character Creation** (B2 + B4 — slot-wheel dials for kindred/class/background/portrait; name field + 🎲; live preview panel showing HP range, speed, class/bg ability names+descriptions, kindred feat name; _build_pc() builds CombatantData from picks; PC added to party before MapScene)
- **Last session (character creation B4 + Test New Run button, 2026-04-24):** B4 — preview `PanelContainer` rendered in a new two-column layout (dials left 3/5, preview right 2/5) on `CharacterCreationScene`. Displays HP range (`10 + kindred_hp + [6..24]`), Speed (`1 + kindred_speed`), per-attribute row (STR/DEX/COG/WIL/VIT all "1–4"), selected class ability name+description, selected background ability name+description, and kindred feat name. `_calc_preview()` upgraded from `{}` stub to a dict-returning function that also pushes values into seven instance-var `Label` refs (per-attribute row built without refs since values don't vary). New helpers `_build_preview_panel()` + `_make_stat_label()` kept inline. `AbilityLibrary` added as a dependency. Also added a **Test New Run** button to `MainMenuScene` — dev shortcut that skips character creation and seeds `GameState.party` with three fully-randomized PCs via `CharacterCreationManager._build_pc()`. Existing 43 headless tests still green; no new tests (pure derived display + dev shortcut).
- **Deferred:** Character creation B3 (Dial widget component — scrapped, not needed for vert slice); portrait serialization (deferred to art pass); Badurga section content (all 6 buttons are stubs); Vendor/Event scene content (NodeStub placeholder); per-ability QTE styles; ability effects are placeholder; boss difficulty scaling from threat quadrants (Feature 8); Feats tab in PartySheet (placeholder); kindred feat mechanical effects (all feats named but have no gameplay effect yet)
