...
- **Last session (Camera pan session, 2026-04-26):** WASD / arrow keys pan orbit pivot in yaw-relative XZ space. Right-click drag now controls full orbit — horizontal = yaw, vertical = elevation pitch (15°–80°, drag up = more top-down). `DRAG_SENSITIVITY` halved to 0.2. Q/E repurposed: raise/lower orbit pivot on world Y axis (not pitch). `focus_on` / `restore` upgraded to parallel tweens that animate pivot position + orbit distance simultaneously; zooms to `QTE_DISTANCE=10.0` on QTE focus, zooms back out on restore. `test_camera_controls.gd` expanded to 6 headless assertions.
