...
- **Last session (events Slice 3 — EventSelector + used_event_ids, 2026-04-24):** `GameState.used_event_ids` (`Array[String]`) added with full save/load/reset wiring. `EventSelector.gd`: static `pick_for_node(ring)` — unseen-first pool, exhaustion fallback, never-null, warning for unrecognized rings, appends to `used_event_ids`, does not call `save()`. `MapManager._move_player_to()`: trailing `GameState.save()` after post-tween dispatch. 7 new headless tests (70 total). EVENT nodes still route to NodeStub.
- **Deferred:** Character creation B3 (Dial widget component — scrapped, not needed for vert slice); portrait serialization (deferred to art pass); Badurga section content (all 6 buttons are stubs); Vendor/Event scene content (NodeStub placeholder — EventScene overlay is Slice 4); per-ability QTE styles; ability effects are placeholder; boss difficulty scaling from threat quadrants (Feature 8); kindred feat mechanical effects (all feats named but have no gameplay effect yet — Slice 4); CombatantData.feats array (Slice 4 — feat_grant event effect); EventScene overlay + effect dispatch + condition evaluator (Slice 4)
