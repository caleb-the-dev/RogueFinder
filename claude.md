...
- **StockGenerator + vendor_stocks persistence** (StockGenerator.roll_stock(vendor, seed_int) — seeded Fisher-Yates + PricingFormula per entry; GameState.vendor_stocks: Dictionary saved/loaded/reset; CITY keyed by vendor_id, WORLD keyed by node_id; MapManager._generate_vendor_stocks() populates on map-gen, no-op on loaded saves; regen_world_vendor_stocks() for future map-reset cycle; 7 headless tests)
- **VendorOverlay UI** (VendorOverlay.gd + VendorOverlay.tscn; CanvasLayer layer 20; show_vendor(instance_key) pulls GameState.vendor_stocks manifest; static try_buy(entry) — validates gold/sold, debits gold, flips sold, adds item to inventory, saves; scrollable stock rows with rarity-colored names + stat summary + gold price + Buy/SOLD states; ESC closes; dev "🛒 Vendor Test" button in MapManager; 6 headless tests)
- **Last session (Vendor Slice 5 — VendorOverlay UI, 2026-04-30):** `VendorOverlay.gd` (`class_name VendorOverlay`, extends CanvasLayer, layer 20). `show_vendor(instance_key: String)` — pulls `GameState.vendor_stocks[key]`, derives VendorData from first entry's vendor_id. `static try_buy(entry: Dictionary) -> bool` — rejects if sold or gold < price; debits `GameState.gold`, flips `entry["sold"] = true`, `add_to_inventory(entry["item"].duplicate())`, `GameState.save()`; returns bool. Scrollable stock rows: rarity-colored name · `_format_stats()` one-liner · gold price · Buy/SOLD. Gold readout + all rows rebuild after each buy. ESC via `_unhandled_input`. Dev "🛒 Vendor Test" button in MapManager `inv_row`. `ui.md` bucket created; `economy.md`, `map.md`, `map_scene.md` updated. 6 headless tests (`test_vendor_buy.gd/.tscn`).
- **Previous session (Vendor Slice 4 — Stock Manifest + Persistence, 2026-04-30):** `StockGenerator.gd` (new — `roll_stock(vendor, seed_int)` seeded Fisher-Yates + `PricingFormula.price_for()` per entry; returns `[{vendor_id, item, price, sold:false}]`). `RewardGenerator._eq_to_dict/_con_to_dict` promoted to public `eq_to_dict/con_to_dict`. `GameState.vendor_stocks: Dictionary = {}` (CITY keyed by vendor_id, WORLD by node_id; `save()`/`load_save()`/`reset()` wired; `regen_world_vendor_stocks()` added). `MapManager._generate_vendor_stocks()` called after `_assign_boss_type()` — no-op on loaded saves, handles old-save migration. 7 headless tests (`test_vendor_stock.gd/.tscn`). Fixed 6 pre-existing GDScript warnings.
- **Previous session (Vendor Slice 2 — PricingFormula, 2026-04-29):** `PricingFormula.price_for(item: Dictionary, rng: RandomNumberGenerator) -> int` — rarity base (COMMON 10 / RARE 25 / EPIC 60 / LEGENDARY 150) × ±20% jitter via caller-supplied RNG; clamped ≥1. Caller supplies RNG for deterministic stock generation from seed. Static helper, no deps.
- **Previous session (Vendor Slice 1 — Currency Reward Channel, 2026-04-29):** `RewardGenerator.gold_drop(ring, threat, avg_level)` added — formula `(RING_BASE[ring] + 0.15×threat + 3.0×avg_level) × randf_range(0.9,1.1)`, clamped ≥1. RING_BASE: outer 30 / middle 20 / inner 12. `GameState.current_combat_ring: String` added as transient (NOT serialized). `MapManager._enter_current_node()` sets `current_combat_ring` via existing `_get_ring()` on COMBAT/BOSS entry. `CombatManager3D._calc_gold_reward()` averages non-dead party levels, converts threat_level (0.0–1.0) → 0–100 int, calls gold_drop, adds result to `GameState.gold`. `EndCombatScreen.show_victory()` extended with `gold_amount: int = 0`; shows "+X Gold (Total: Y)" line in gold color above item cards. 7 headless tests (`test_gold_reward.gd/.tscn`).
- **Previous session (ATK Stat Removal, 2026-04-29):** `CombatantData.attack` computed property removed — no separate attack stat exists. New `effective_stat(stat: String) -> int` method returns raw attribute + all 6 bonus sources (equip/feat/class/kindred/bg/temp). HARM formula in `CombatManager3D._run_harm_defenders` now uses `_get_attribute_value(caster, ability.attribute)` → `effective_stat()` — so a STR ability scales with effective STR, COG with effective COG, etc. `PartySheet` derived stats row reduced from 6 to 5 cols (Atk removed). `StatPanel` Attack line removed. GAME_BIBLE, combatant_data.md, combat_manager.md, party_sheet.md all updated.
...
- **Deferred:** Character creation B3 (Dial widget component — scrapped); portrait serialization (deferred to art pass); Badurga remaining 6 sections (Tavern/Bulletin/Weapons/Armor/Accessories/Consumables vendors are stubs); **Vendor Slice 6 — wire-up** (VendorOverlay not yet called from map VENDOR node entry or Badurga shop stalls; NodeStub placeholder still shown on VENDOR nodes; dev button in MapManager to be removed); ability effects are placeholder; boss difficulty scaling from threat quadrants (Feature 8); trigger-based feat effects (effects JSON column is empty — data-layer only); feat rarity/tiering/swapping; enemy `qte_resolution` retune; level-up stat bonuses (level grants ability/feat pick only — no automatic stat increases yet); audio bus wiring for volume sliders (sliders persist values but don't affect actual audio yet); regen_world_vendor_stocks() trigger not wired (hook exposed but boss-win cycle not implemented yet)
