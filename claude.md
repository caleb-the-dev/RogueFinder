...
- **Last session (S25):** Map node placement constraints. Inner ring: 4 COMBAT, 2 EVENT (no VENDOR). Outer ring: 7 COMBAT, 3 EVENT, 1 VENDOR, 1 BOSS. BOSS deferred to `_assign_boss_type()` (runs after `_build_edge_data()`); guaranteed ≥ 2 ring-hops from any middle→outer bridge. Exactly 2 VENDOR per run (1 middle, 1 outer). S24: RECRUIT node type removed entirely from MapManager and all doc files.
