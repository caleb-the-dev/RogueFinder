...
- **Last session (Pause Menu + Archetypes Log, 2026-04-28):** `PauseMenuManager.gd` added as CanvasLayer autoload (layer 26, `PROCESS_MODE_ALWAYS`). ESC + ☰ button open the menu from gameplay scenes; blocked via `_scene_name_is_pauseable()` on main.tscn + MainMenuScene + RunSummaryScene. Sub-panels: Resume / Settings / Guide / Archetypes Log / confirm dialogs (Main Menu + Exit). `SettingsStore.gd` added (volume prefs persist to `user://settings.json`; fullscreen dropped — CheckBox rendered as non-interactive text). `GameState.recruited_archetypes: Array[String]` + `record_recruited_archetype()` added (save/load/reset wired; typed-array); `add_to_bench()` calls it — single hookpoint for all follower paths. Archetypes Log: shows all 9 archetypes sorted by status — UNKNOWN (dark silhouette "???"), ENCOUNTERED (blue badge), FOLLOWER (green badge), PLAYER = RogueFinder (gold "[ You ]" / "The Pathfinder"). CM3D ESC: only consumed when action taken; IDLE falls through to PauseMenu. Party button shifted left (−236 px) to clear ☰ overlap. 12 headless tests (`test_pause_menu.gd/.tscn`).
