...
- **Live systems:** 3D combat loop · traversable world map with 5 node types (COMBAT, VENDOR, EVENT, BOSS, CITY) + structured placement rules · save/load · reward system · Badurga city shell (placeholder sections) · threat escalation counter + HUD bar · persistent party (HP/energy carry-over, ally permadeath, run-end summary screen) · party bag inventory (all reward items land in shared bag; equipment + consumables stored as raw dicts) · party sheet overlay (interactive 4-quadrant equip UI, layer 20, full ability pool swap with drag-compare) · CombatActionPanel (right slide-in, player + enemy view, tooltips) · **Kindreds** (species/ancestry field on all characters — persisted, shown in StatPanel, CombatActionPanel, and PartySheet)
- **Last session (S28):** Added `kindred: String` to `CombatantData`. Each archetype has a fixed kindred (Human/Human/Half-Orc/Gnome/Dwarf). Persisted in save/load; old saves default to `"Unknown"`. Kindred shown in StatPanel (between Archetype and Background), CombatActionPanel (small muted label below unit name), and PartySheet card TOP-LEFT. PartySheet column widths rebalanced to ~30/40/30 (bag/character/abilities). HP row restructured to inline text+bar on one line. GAME_BIBLE updated.
- **Deferred:** Badurga section content (all 6 buttons are stubs), Vendor/Event scene content (NodeStub placeholder), per-ability QTE styles, ability effects are placeholder, boss difficulty scaling from threat quadrants (Feature 8), dedicated main menu / title scene (RunSummaryScene "Main Menu" button is a stub — same as Start New Run), Feats tab (placeholder)
