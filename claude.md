...
- **Last session (QTE reactive overhaul Session B, 2026-04-26):** World-space bar. `start_qte(energy_cost, attacker: Node3D)` — bar now floats above the attacker's head each frame via `Camera3D.unproject_position(attacker.global_position + Vector3(0, 2.0, 0))`. Full-screen dark overlay removed. `_attacker` cleared to null after `qte_resolved` fires. Attacker-death guard in `_process`: if attacker invalid/dead mid-QTE, bar hides and emits 0.25 (miss). All 3 headless test suites pass.
